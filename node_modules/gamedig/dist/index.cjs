...
var Core = class extends import_node_events3.EventEmitter {
  ...
  async runOnce() {
    ...
    if (!("connect" in state)) {
      state.connect = "" + (state.gameHost || this.options.host || this.options.address) + ":" + (state.gamePort || this.options.port);
    }
    ...
    while (!reader.done()) {
      ...
      for (let iField = 0; iField < fields.length; iField++) {
        ...
        if (key === "player_")
          ...
          key = "tickets";
        if (key === "score" || key === "deaths" || key === "ping" || key === "team" || key === "kills" || key === "tickets") {
          if (value === "")
            continue;
          value = parseInt(value);
        }
...
var palworld = class extends Epic {
  constructor() {
    super();
